...
import { ServerCog, Save, Mic, Settings2, ShieldCheck, ChevronDown, FolderOpen, Sword, Skull, RefreshCw } from 'lucide-react';
...
function ChannelSelect({ label, icon, value, onChange, options = [], placeholder, loading }) {
  const currentValueInOptions = options?.some(opt => String(opt.id) === String(value));
        ...
        {loading ? (
          ...
          <div className="relative group">
            <select 
              ...
              className="w-full appearance-none bg-black/60 border border-white/10 rounded-xl px-4 py-3 text-slate-200 font-semibold focus:border-blue-500/50 focus:ring-4 focus:ring-blue-500/5 outline-none transition-all cursor-pointer group-hover:border-white/20 relative z-20"
              style={{ pointerEvents: 'auto' }}
            >
              <option value="" className="bg-[#0a0a0f]">{placeholder}</option>
              
              {/* Resiliency Fallback: If Value exists but isn't in options yet */}
              {value && !currentValueInOptions && (
                <option value={value} className="bg-[#0a0a0f] text-blue-400 italic">
                  [Manifesting ID: {value}...]
                </option>
              )}

              {options.map(ch => (
                <option key={ch.id} value={ch.id} className="bg-[#0a0a0f]">
                  {ch.name}
                </option>
              ))}
            </select>
            <ChevronDown size={16} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 pointer-events-none group-hover:text-slate-300 transition-colors z-30" />
          </div>
        )}
      </div>
      {value && !loading && (
        <p className="absolute bottom-0 left-0 text-[10px] text-blue-500/40 font-mono tracking-wider">SIGIL ID: {value}</p>
      )}
    </div>
  );
}
...
export default function TempVoiceSetup() {
  ...
  const [retryKey, setRetryKey]   = useState(0); // Incrementing triggers re-fetch
  ...
  useEffect(() => {
    ...
    setChLoading(true);
    setError(null);
    setChannels({ voiceChannels: [], categories: [], textChannels: [] });
    setConfig({
      join_to_create_channel: '',
      temp_vc_category:       '',
      default_user_limit:     0,
    ...
    const manifestData = async () => {
      const force = retryKey > 0 ? '?force=true' : '';
      try {
        const [configRes, channelRes] = await Promise.all([
          axios.get(`${apiUrl}/guilds/${id}/config${force}`, { headers }),
          axios.get(`${apiUrl}/guilds/${id}/channels${force}`, { headers })
        ]);
        ...
        console.error("Ritual Sync Failed:", err.response?.status, err.response?.data || err.message);
        const status = err.response?.status;
        const serverError = err.response?.data?.error;

        // Always prefer the backend's specific message — it now tells the exact cause.
        if (status === 401) {
          setError(serverError || "Hunter session expired. Please log out and log back in.");
        } else if (status === 429) {
          setError(serverError || "Discord rate limit hit. Wait a moment, then click Try Re-Scrying.");
        } else if (status === 503 || status === 504 || !err.response) {
          setError("Gateway Latency: The portal to Discord is temporarily unstable. Try again shortly.");
        } else {
          setError(serverError || "Connection to sanctuary lost.");
        }
      } finally {
        setLoading(false);
        setChLoading(false);
  ...
  if (error) return (
    <div className="flex flex-col items-center justify-center min-h-[40vh] gap-6 p-8 text-center">
      ...
      <div className="space-y-2">
        ...
        <p className="text-slate-500 text-sm max-w-sm">Determine if the Scribe's spirit has been banished from this guild or if the gateway has collapsed.</p>
      </div>
      <DungeonButton variant="mana" onClick={() => { setError(null); setRetryKey(k => k + 1); }} icon={RefreshCw}>Try Re-Scrying</DungeonButton>
    </div>
  );
