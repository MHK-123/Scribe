...
export default function Login() {
  const { user, apiUrl, discordAuthUrl, isRateLimited } = useContext(AuthContext);
    ...
    <div className="min-h-screen w-full flex items-center justify-center relative overflow-hidden" style={{background:'#020209'}}>
      ...
      >
        ...
        <div className="glass-card p-10 text-center" style={{
          ...
          {/* Cooldown Alert Manifestation */}
          {isRateLimited && (
            <motion.div
              initial={{ height: 0, opacity: 0 }}
              animate={{ height: 'auto', opacity: 1 }}
              className="mb-8 p-4 rounded-lg bg-red-500/10 border border-red-500/30 text-red-400 text-xs text-left space-y-2"
            >
              <div className="flex items-center gap-2 font-bold tracking-widest uppercase">
                <span className="w-2 h-2 bg-red-500 rounded-full animate-pulse"/>
                Portal Censored (1015)
              </div>
              <p className="opacity-80 leading-relaxed italic">
                Your sanctuary's gateway is currently rate-limited by Cloudflare. 
                Please wait 5-10 minutes before re-igniting the handshake.
              </p>
            </motion.div>
          )}
          ...
          >
            ...
            >
              ...
              {isRateLimited ? 'Cooldown Active' : 'Authenticate via Discord'}
            </DungeonButton>
          </motion.div>
          ...
          >
            <div className="flex items-center gap-4 text-[10px] font-bold tracking-widest uppercase">
              <Link to="/terms" className="text-slate-500 hover:text-purple-400 transition-colors">Manifesto</Link>
              <span className="w-1 h-1 bg-white/10 rounded-full" />
              <Link to="/privacy" className="text-slate-500 hover:text-blue-400 transition-colors">Privacy</Link>
            </div>
            <p style={{color:'rgba(136,146,176,0.3)', fontSize:'0.7rem', letterSpacing:'0.04em'}}>
              SCRIBE v2.1 · ARCANE DOCUMENTATION INCLUDED
            </p>
          </motion.div>
