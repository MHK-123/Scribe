          ...
          <div className="flex-1 w-full space-y-3">
            ...
            <div className="relative group">
               <select 
                  ...
                  className="w-full appearance-none bg-black/60 border border-white/10 rounded-xl px-4 py-4 text-white font-bold outline-none focus:border-blue-500/40 transition-all cursor-pointer relative z-20"
                  style={{ pointerEvents: 'auto' }}
               >
                  <option value="" disabled className="bg-[#0a0a0f]">{error ? `⚠️ ${error}` : 'Select a role...'}</option>
                  {(roles || []).map(r => (
               ...
               <div className="absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none text-slate-500 group-hover:text-slate-300 transition-colors z-30">▼</div>
            </div>
          </div>
