...
router.get('/:guildId/configs', async (req, res) => {
  try {
  ...
  try {
    ...
    const { rows } = await pool.query(
      ...
      [guildId, voice_channel_id, text_channel_id,
       focus_duration, break_duration, cycles, auto_start, auto_stop]
    );
    res.json(rows[0]);
  } catch (err) {
...
router.delete('/:guildId/config/:id', async (req, res) => {
  try {
    await pool.query(
...
router.get('/:guildId/sessions', async (req, res) => {
  try {
    const { rows } = await pool.query(
